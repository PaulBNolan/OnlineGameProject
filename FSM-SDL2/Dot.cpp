...
void Dot::Init(SDL_Renderer* gRenderer)
{
	if (isChaser)
	{
		if (!gDotTexture.loadFromFile("ASSETS/IMAGES/reddot.bmp", gRenderer))
		{
			printf("Failed to load dot texture!\n");
		}
	}
	else {
		if (!gDotTexture.loadFromFile("ASSETS/IMAGES/bluedot.bmp", gRenderer))
		{
			printf("Failed to load dot texture!\n");
		}
	}
...
void Dot::setOtherPosition()
{
	not_digit notADigit;

	std::string::iterator end = std::remove_if(m_textPos.begin(), m_textPos.end(), notADigit);

	std::string all_numbers(m_textPos.begin(), end);
